...
<body>
  ...
  <!----------****Edite Action Start****------------->
					 ...
					 if(isset($_GET["edit_id"]))
					 {
						 ...
						 $query = mysqli_query($connection,"select * from cake_item_table where Cake_Id='".$editcakeid."'");
						 $tbl_row = mysqli_fetch_array($query);						
						 $cake_name = $tbl_row["Cake_name"];
						 $cake_flavour = $tbl_row["Cake_flavour"];
						 $proimg = $tbl_row["Cake_Image"];
						 $cake_des = $tbl_row["Cake_Des"];
						 $cake_price = $tbl_row["Cake_Price"];
						 $catid_fk = $tbl_row["Cat_id_fk"];  
...
<!----------****Edite Action End****------------->
       ...
       <!----------****Insert Action Start****------------->
						...
						if(!empty($_POST["hideid"]))
				{
					if (empty($_FILES["fileupload"]["name"]))
					{
						...
						$query = "update cake_item_table set 
						Cake_name='".$item_name."',
						Cake_flavour='".$item_flavour."',
						Cake_Image='".$mypath."',
						Cake_Des='".$item_des."',
						Cake_Price='".$item_price."',
						Cat_id_fk='".$item_category."'
						where Cake_Id='".$hide_id."'";
       ...
       <!----------****Insert Action End****------------->
                    ...
                    <form method="post" action="Add_Items.php" enctype="multipart/form-data">
                     ...
                     <img src="/images/<?php echo @$proimg ?>" style="visibility:<?php echo @$imgvisible ?>" class="img-thumbnail" width="300" height="300" id="cakeimg"/>
                        </div>
                        </div>
